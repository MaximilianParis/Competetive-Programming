...
int n, m, d;
vector<int>nums;
vector<tuple<int, int, int, int>> CountZeroAndOnes;
ll ans = 0;
int x = 0;
pair<ll, ll> InversionsAtBit(vector<int>& values, int bit, int value,int pos);
void dp(vector<int>& values) {
    vector<vector<int>> BitValues1(n);
    vector<vector<int>> BitValues2(n);
    CountZeroAndOnes = vector<tuple<int, int, int, int>>(n);
    ...
    for (int bit = maxbit; bit >= 0; bit--) {
        ...
        if (bit == maxbit) {
            ...
            pair<ll, ll> bitValues = InversionsAtBit(values, bit, 0,0);
            if (bitValues.first <= bitValues.second) {
                ...
                for (int j = 0; j <= size; j++) {
                    ...
                    int cntZero = get<2>(CountZeroAndOnes[j]);
                    int cntOne = get<3>(CountZeroAndOnes[j]);
            ...
            for (int j = 0; j < count; j++) {
                ...
                pair<ll, ll> bitValues1 = InversionsAtBit(BitValues[j], bit, x,j);
...
pair<ll, ll> InversionsAtBit(vector<int>& values, int bit, int value, int pos) {
    ...
    get<2>(CountZeroAndOnes[pos]) = cntZero;
    get<3>(CountZeroAndOnes[pos]) = cntOne;
